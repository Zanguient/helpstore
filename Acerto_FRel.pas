...
uses
  ...
  ppClass, ppCache, ppComm, ppRelatv, ppProd, ppReport, ppStrtch, ppSubRpt,
    ppRegion, cxPropertiesStore,  Registry, cxGraphics, cxControls,
...
type
  TFRelAcerto = class(TForm)
    ...
    GroupBox1: TGroupBox;
    edtEmissaoFinal: TdxDateEdit;
    Label2: TcxLabel;
    Label3: TcxLabel;
    edtEmissaoInicial: TdxDateEdit;
    Label7: TcxLabel;
    Label8: TcxLabel;
    GroupBox5: TGroupBox;
    Label1: TcxLabel;
...
procedure TFRelAcerto.pnlClientResize(Sender: TObject);
begin
  b2.Width := pnlClient.Width - 17;
end;

procedure TFRelAcerto.ActFecharExecute(Sender: TObject);
begin
     Modalresult := mrCancel ;
end;

procedure TFRelAcerto.IniKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TFRelAcerto.ActPreviewExecute(Sender: TObject);
var
...
procedure TFRelAcerto.FormShow(Sender: TObject);
begin
...
procedure TFRelAcerto.IniEnter(Sender: TObject);
begin
     CORFUNDO( SENDER );
end;

procedure TFRelAcerto.IniExit(Sender: TObject);
begin
     //TROCA A COR PARA A COR SELECIONADA
     TIRACORFUNDO( SENDER );
end;

procedure TFRelAcerto.Rd1Click(Sender: TObject);
begin
 if Rd1.Checked then
 begin
...
procedure TFRelAcerto.CmbCobradorExit(Sender: TObject);
begin
  TIRACORFUNDO(SENDER);
end;

procedure TFRelAcerto.CmbCobradorEnter(Sender: TObject);
begin
  CORFUNDO(SENDER);
end;

procedure TFRelAcerto.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if (key=VK_RETURN) OR (Key=VK_DOWN) Then
       Perform(WM_NEXTDLGCTL, 0, 0);
...
procedure TFRelAcerto.FormCreate(Sender: TObject);
begin
...
procedure TFRelAcerto.FormClose(Sender: TObject;
...
procedure TFRelAcerto.rd2Click(Sender: TObject);
begin
 if Rd2.Checked then
 begin
...
procedure TFRelAcerto.ppDBCalc33Print(Sender: TObject);
begin
...
procedure TFRelAcerto.ppDBCalc26Print(Sender: TObject);
begin
  if (TppDBCalc(Sender).Value < 0) then
...
procedure TFRelAcerto.ppDetailBand6BeforePrint(Sender: TObject);
begin
