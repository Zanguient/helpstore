...
procedure TFrmAdministradoras.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  //
  ...
  //
  If FrmAdministradoras.FormStyle = fsMDIChild Then
...
begin
  If MessageBox(Handle,'Tem certeza que deseja Excluir esta Administradora?',
                       'Atenção !!!',MB_YESNO+MB_ICONQUESTION+MB_DEFBUTTON2)=IDYES Then
     DataSource.Dataset.Delete;
end;

procedure TFrmAdministradoras.ActPostExecute(Sender: TObject);
begin
  Try
...
procedure TFrmAdministradoras.ActCancelExecute(Sender: TObject);
begin
...
procedure TFrmAdministradoras.FormShow(Sender: TObject);
begin
...
procedure TFrmAdministradoras.ActListagemExecute(Sender: TObject);
begin
  //
  DMListagens := TDMListagens.Create(Self);
  DMListagens.lsAdministradoras(DataSource.DataSet);
  DMListagens.Free;
  DMListagens := Nil;
end;

procedure TFrmAdministradoras.ActLocalizarExecute(Sender: TObject);
begin
  try
        ...
        Frm_Localizar_Generico.LblTitulo.Caption := 'Localizar Administradoras' ;
...
procedure TFrmAdministradoras.dxDBEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmAdministradoras.pnlClientResize(Sender: TObject);
begin
  b2.Width := pnlClient.Width - 17;
  LblRegistros.Left := pnlClient.Width - 11 - LblRegistros.Width;
end;

procedure TFrmAdministradoras.DataSourceStateChange(Sender: TObject);
begin
  If DataSource.State in dsEditModes Then
     Begin
...
procedure TFrmAdministradoras.ActFecharExecute(Sender: TObject);
begin
  If FrmAdministradoras.FormStyle = fsMDIChild Then
...
procedure TFrmAdministradoras.DataSourceDataChange(Sender: TObject; Field: TField);
begin
  LblRegistros.Caption := IntToStr(Datasource.DataSet.Recno) + '/' + IntToStr(Datasource.Dataset.RecordCount); 
end;

procedure TFrmAdministradoras.EdInicialEnter(Sender: TObject);
begin
     CORFUNDO ( SENDER );
end;

procedure TFrmAdministradoras.EdInicialExit(Sender: TObject);
begin
     TIRACORFUNDO ( SENDER );
end;
