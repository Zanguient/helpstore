...
{  If DMCadastros.Tag = 0 Then
  ...
  If FrmAtividades.FormStyle = fsMDIChild Then
...
procedure TFrmAtividades.ActIncluirExecute(Sender: TObject);
begin
...
procedure TFrmAtividades.ActExcluirExecute(Sender: TObject);
begin
  If MessageBox(Handle,'Tem certeza que deseja Excluir esta Atividade ?',
                       'Atenção !!!',MB_YESNO+MB_ICONQUESTION+MB_DEFBUTTON2)=IDYES Then
     DataSource.Dataset.Delete;
end;

procedure TFrmAtividades.ActPostExecute(Sender: TObject);
begin
  Try
...
procedure TFrmAtividades.ActCancelExecute(Sender: TObject);
begin
...
procedure TFrmAtividades.ActListagemExecute(Sender: TObject);
begin
  //
  DMListagens := TDMListagens.Create(Self);
  DMListagens.lsAtividades(Datasource.DataSet);
  DMListagens.Free;
  DMListagens := Nil;
end;

procedure TFrmAtividades.ActLocalizarExecute(Sender: TObject);
begin
  try
        ...
        Frm_Localizar_Generico.LblTitulo.Caption := 'Localizar Atividades' ;
...
procedure TFrmAtividades.dxDBEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmAtividades.pnlClientResize(Sender: TObject);
begin
  b2.Width := pnlClient.Width - 17;
  LblRegistros.Left := pnlClient.Width - 11 - LblRegistros.Width;
end;

procedure TFrmAtividades.DataSourceStateChange(Sender: TObject);
begin
  If DataSource.State in dsEditModes Then
     Begin
...
procedure TFrmAtividades.ActFecharExecute(Sender: TObject);
begin
  If FrmAtividades.FormStyle = fsMDIChild Then
...
procedure TFrmAtividades.DataSourceDataChange(Sender: TObject; Field: TField);
begin
  LblRegistros.Caption := IntToStr(Datasource.DataSet.Recno) + '/' + IntToStr(Datasource.Dataset.RecordCount);
end;

procedure TFrmAtividades.ActAlterarExecute(Sender: TObject);
begin
  If DataSource.State in dsEditModes Then
     Exit;
  Try
...
procedure TFrmAtividades.EdInicialEnter(Sender: TObject);
begin
     CORFUNDO ( SENDER );
end;
