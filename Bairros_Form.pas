...
{  If DMCadastros.Tag = 0 Then
     ...
     End;}
  //
  If FrmBairros.FormStyle = fsMDIChild Then
...
procedure TFrmBairros.ActIncluirExecute(Sender: TObject);
begin
...
procedure TFrmBairros.ActExcluirExecute(Sender: TObject);
begin
...
procedure TFrmBairros.ActPostExecute(Sender: TObject);
begin
  Try
...
procedure TFrmBairros.ActCancelExecute(Sender: TObject);
begin
...
procedure TFrmBairros.ActListagemExecute(Sender: TObject);
begin
  //
...
procedure TFrmBairros.ActLocalizarExecute(Sender: TObject);
begin
  try
...
procedure TFrmBairros.dxDBEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmBairros.pnlClientResize(Sender: TObject);
begin
  b2.Width := pnlClient.Width - 17;
  LblRegistros.Left := pnlClient.Width - 11 - LblRegistros.Width;
end;

procedure TFrmBairros.DataSourceStateChange(Sender: TObject);
begin
  If DataSource.State in dsEditModes Then
     Begin
...
procedure TFrmBairros.ActFecharExecute(Sender: TObject);
begin
  If FrmBairros.FormStyle = fsMDIChild Then
...
procedure TFrmBairros.DataSourceDataChange(Sender: TObject; Field: TField);
begin
  LblRegistros.Caption := IntToStr(Datasource.DataSet.Recno) + '/' + IntToStr(Datasource.Dataset.RecordCount); 
end;

procedure TFrmBairros.ActAlterarExecute(Sender: TObject);
begin
  If DataSource.State in dsEditModes Then
     Exit;
  Try
...
procedure TFrmBairros.EdInicialEnter(Sender: TObject);
begin
     CORFUNDO ( SENDER );
end;

procedure TFrmBairros.EdInicialExit(Sender: TObject);
begin
     TIRACORFUNDO ( SENDER );
end;
