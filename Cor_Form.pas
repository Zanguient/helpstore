...
{  If DMCadastros.Tag = 0 Then
     ...
     End;}
  //
  If FrmCores.FormStyle = fsMDIChild Then
...
procedure TFrmCores.ActIncluirExecute(Sender: TObject);
begin
...
procedure TFrmCores.ActExcluirExecute(Sender: TObject);
begin
...
procedure TFrmCores.ActPostExecute(Sender: TObject);
begin
  Try
...
procedure TFrmCores.ActCancelExecute(Sender: TObject);
begin
...
procedure TFrmCores.ActListagemExecute(Sender: TObject);
begin
  //
  DMListagens := TDMListagens.Create(Self);
  DMListagens.lsCores(DataSource.DataSet);
  DMListagens.Free;
  DMListagens := Nil;
end;

procedure TFrmCores.ActLocalizarExecute(Sender: TObject);
begin
  try
        ...
        Frm_Localizar_Generico.LblTitulo.Caption := 'Localizar Cores' ;
...
procedure TFrmCores.dxDBEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmCores.pnlClientResize(Sender: TObject);
begin
  b2.Width := pnlClient.Width - 17;
  LblRegistros.Left := pnlClient.Width - 11 - LblRegistros.Width;
end;

procedure TFrmCores.DataSourceStateChange(Sender: TObject);
begin
  If DataSource.State in dsEditModes Then
     Begin
...
procedure TFrmCores.ActFecharExecute(Sender: TObject);
begin
  If FrmCores.FormStyle = fsMDIChild Then
...
procedure TFrmCores.DataSourceDataChange(Sender: TObject; Field: TField);
begin
  LblRegistros.Caption := IntToStr(Datasource.DataSet.Recno) + '/' + IntToStr(Datasource.Dataset.RecordCount); 
end;

procedure TFrmCores.ActAlterarExecute(Sender: TObject);
begin
  If DataSource.State in dsEditModes Then
     Exit;
  Try
...
procedure TFrmCores.EdInicialEnter(Sender: TObject);
begin
     CORFUNDO ( SENDER );
end;

procedure TFrmCores.EdInicialExit(Sender: TObject);
begin
     TIRACORFUNDO ( SENDER );
end;
