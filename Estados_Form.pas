...
{  If DMCadastros.Tag = 0 Then
     ...
     End;}
  //
  If FrmEstados.FormStyle = fsMDIChild Then
...
procedure TFrmEstados.ActPostExecute(Sender: TObject);
begin
  Try
...
procedure TFrmEstados.ActCancelExecute(Sender: TObject);
begin
...
procedure TFrmEstados.ActListagemExecute(Sender: TObject);
begin
  //
...
procedure TFrmEstados.ActLocalizarExecute(Sender: TObject);
begin
  DMApp.Localizar(Datasource);
end;

procedure TFrmEstados.EdInicialKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmEstados.pnlClientResize(Sender: TObject);
begin
  b2.Width := pnlClient.Width - 17;
  LblRegistros.Left := pnlClient.Width - 11 - LblRegistros.Width;
end;

procedure TFrmEstados.DataSourceStateChange(Sender: TObject);
begin
  If DataSource.State in dsEditModes Then
     Begin
...
procedure TFrmEstados.ActFecharExecute(Sender: TObject);
begin
  If FrmEstados.FormStyle = fsMDIChild Then
...
procedure TFrmEstados.DataSourceDataChange(Sender: TObject; Field: TField);
begin
  LblRegistros.Caption := IntToStr(Datasource.DataSet.Recno) + '/' + IntToStr(Datasource.Dataset.RecordCount);
end;

procedure TFrmEstados.EdInicialEnter(Sender: TObject);
begin
     CORFUNDO ( SENDER );
end;

procedure TFrmEstados.EdInicialExit(Sender: TObject);
begin
     TIRACORFUNDO ( SENDER );
end;
