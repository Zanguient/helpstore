object FrmOrcamentos_Mascaras: TFrmOrcamentos_Mascaras
  Left = 242
  Top = 91
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'M'#225'scara Para Or'#231'amentos'
  ClientHeight = 438
  ClientWidth = 610
  Color = 15396334
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object pnlClient: TPanel
    Left = 0
    Top = 0
    Width = 610
    Height = 438
    Align = alClient
    BevelInner = bvRaised
    BevelOuter = bvLowered
    Color = clWhite
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    OnResize = pnlClientResize
    object Bevel1: TBevel
      Left = 2
      Top = 404
      Width = 606
      Height = 2
      Align = alBottom
      Shape = bsTopLine
    end
    object b2: TBevel
      Left = 8
      Top = 35
      Width = 184
      Height = 2
      Shape = bsTopLine
    end
    object Label1: TcxLabel
      Left = 79
      Top = 56
      Caption = 'C'#243'digo'
      ParentFont = False
      Transparent = True
    end
    object Label2: TcxLabel
      Left = 85
      Top = 81
      Caption = 'Nome'
      ParentFont = False
      Transparent = True
    end
    object LblTitulo: TcxLabel
      Left = 6
      Top = 8
      Caption = 'M'#225'scara para Or'#231'amento'
      ParentFont = False
      Transparent = True
    end
    object Label4: TcxLabel
      Left = 9
      Top = 189
      Caption = 'Nome'
      ParentFont = False
      Transparent = True
    end
    object Label5: TcxLabel
      Left = 172
      Top = 189
      Caption = 'Grupo'
      ParentFont = False
      Transparent = True
    end
    object Label6: TcxLabel
      Left = 398
      Top = 189
      Caption = 'Sub-Grupo'
      ParentFont = False
      Transparent = True
    end
    object pnlBottom: TPanel
      Left = 2
      Top = 406
      Width = 606
      Height = 30
      Align = alBottom
      BevelOuter = bvNone
      TabOrder = 3
      object dxBarDockControl1: TdxBarDockControl
        Left = 0
        Top = 2
        Width = 606
        Height = 26
        Align = dalTop
        BarManager = BarManager
      end
      object Panel1: TPanel
        Left = 0
        Top = 0
        Width = 606
        Height = 2
        Align = alTop
        BevelOuter = bvNone
        TabOrder = 1
      end
    end
    object edCodigo: TdxDBEdit
      Left = 119
      Top = 51
      Width = 74
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 0
      OnKeyDown = edCodigoKeyDown
      DataField = 'CODIGO'
      DataSource = DsAgrupador
      ReadOnly = True
      StoredValues = 64
    end
    object EdInicial: TdxDBEdit
      Left = 119
      Top = 76
      Width = 290
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      Style.BorderColor = clBtnShadow
      Style.BorderStyle = xbsSingle
      Style.HotTrack = True
      TabOrder = 1
      OnEnter = EdInicialEnter
      OnExit = EdPorcentagemExit
      OnKeyDown = edCodigoKeyDown
      CharCase = ecUpperCase
      DataField = 'NOME'
      DataSource = DsAgrupador
    end
    object GRID: TDBCtrlGrid
      Left = 5
      Top = 204
      Width = 596
      Height = 196
      AllowDelete = False
      AllowInsert = False
      DataSource = DataSource
      PanelHeight = 28
      PanelWidth = 580
      TabOrder = 4
      RowCount = 7
      SelectedColor = clBtnFace
      object DBText5: TDBText
        Left = 168
        Top = 5
        Width = 225
        Height = 17
        DataField = 'NOME_GRUPO'
        DataSource = DataSource
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 8404992
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object DBText7: TDBText
        Left = 392
        Top = 5
        Width = 182
        Height = 17
        DataField = 'NOME_SUB_GRUPO'
        DataSource = DataSource
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 8404992
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object DBText8: TDBText
        Left = 5
        Top = 5
        Width = 156
        Height = 17
        DataField = 'NOME'
        DataSource = DataSource
        Font.Charset = DEFAULT_CHARSET
        Font.Color = 8404992
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
    end
    object Panel2: TPanel
      Left = 4
      Top = 104
      Width = 593
      Height = 81
      TabOrder = 2
      OnEnter = Panel2Enter
      object Label8: TcxLabel
        Left = 8
        Top = 42
        Caption = 'Grupo'
        ParentFont = False
        Transparent = True
      end
      object Label3: TcxLabel
        Left = 277
        Top = 42
        Caption = 'Sub-Grupo'
        ParentFont = False
        Transparent = True
      end
      object Label7: TcxLabel
        Left = 8
        Top = 2
        Caption = 'Nome'
        ParentFont = False
        Transparent = True
      end
      object BtnCadastrar: TcxButton
        Left = 516
        Top = 53
        Width = 73
        Height = 25
        Cursor = crHandPoint
        Caption = '&Salvar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 3
        OnClick = BtnCadastrarClick
        Glyph.Data = {
          
          0000}
      end
      object CmbGrupo: TdxDBLookupEdit
        Left = 8
        Top = 54
        Width = 265
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Style.BorderColor = clBtnShadow
        Style.BorderStyle = xbsSingle
        Style.ButtonStyle = btsSimple
        Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
        Style.HotTrack = True
        Style.Shadow = False
        TabOrder = 1
        OnEnter = EdInicialEnter
        OnExit = CmbGrupoExit
        OnKeyDown = edCodigoKeyDown
        DataField = 'GRUPO'
        DataSource = DataSource
        ReadOnly = False
        DropDownWidth = 294
        ClearKey = 8
        ListFieldName = 'NOME'
        Revertable = True
        KeyFieldName = 'CODIGO'
        ListSource = DsGrupo
        LookupKeyValue = Null
        StoredValues = 64
      end
      object dxDBLookupEdit1: TdxDBLookupEdit
        Left = 274
        Top = 54
        Width = 239
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Style.BorderColor = clBtnShadow
        Style.BorderStyle = xbsSingle
        Style.ButtonStyle = btsSimple
        Style.Edges = [edgLeft, edgTop, edgRight, edgBottom]
        Style.HotTrack = True
        Style.Shadow = False
        TabOrder = 2
        OnEnter = EdInicialEnter
        OnExit = CmbGrupoExit
        OnKeyDown = edCodigoKeyDown
        DataField = 'SUB_GRUPO'
        DataSource = DataSource
        ReadOnly = False
        DropDownWidth = 294
        ClearKey = 8
        ListFieldName = 'NOME'
        Revertable = True
        KeyFieldName = 'SUBGRUPO'
        ListSource = DsSubgrupo
        LookupKeyValue = Null
        StoredValues = 64
      end
      object ednome: TdxDBEdit
        Left = 8
        Top = 15
        Width = 580
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
        Style.BorderColor = clBtnShadow
        Style.BorderStyle = xbsSingle
        Style.HotTrack = True
        TabOrder = 0
        OnEnter = EdInicialEnter
        OnExit = EdPorcentagemExit
        OnKeyDown = edCodigoKeyDown
        CharCase = ecUpperCase
        DataField = 'NOME'
        DataSource = DataSource
      end
    end
  end
  object BarDBNavigator: TdxBarDBNavigator
    BarManager = BarManager
    CategoryName = 'DB Navigator'
    ConfirmDelete = False
    DataSource = DsAgrupador
    DBCheckLinks = <>
    VisibleButtons = [dxbnPrior, dxbnNext]
    Left = 205
    Top = 224
  end
  object BarManager: TdxBarManager
    AllowReset = False
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    CanCustomize = False
    Categories.Strings = (
      'Default'
      'DB Navigator')
    Categories.ItemsVisibles = (
      2
      2)
    Categories.Visibles = (
      True
      True)
    PopupMenuLinks = <>
    Style = bmsFlat
    UseSystemFont = True
    Left = 341
    Top = 224
    DockControlHeights = (
      0
      0
      0
      0)
    object BarManagerBar1: TdxBar
      AllowClose = False
      AllowCustomizing = False
      AllowQuickCustomizing = False
      AllowReset = False
      Caption = 'Navega'#231#227'o'
      CaptionButtons = <>
      DockControl = dxBarDockControl1
      DockedDockControl = dxBarDockControl1
      DockedLeft = 0
      DockedTop = 0
      FloatLeft = 290
      FloatTop = 163
      FloatClientWidth = 23
      FloatClientHeight = 198
      ItemLinks = <
        item
          Visible = True
          ItemName = 'dxBarBDBNavPrev'
        end
        item
          Visible = True
          ItemName = 'dxBarBDBNavNext'
        end
        item
          Visible = True
          ItemName = 'btnIncluir'
        end
        item
          Visible = True
          ItemName = 'btnExcluir'
        end
        item
          Visible = True
          ItemName = 'btnLocalizar'
        end
        item
          Visible = True
          ItemName = 'BtnSalvar'
        end
        item
          Visible = True
          ItemName = 'BtnCancelar'
        end
        item
          Visible = True
          ItemName = 'FecharAlteracao'
        end
        item
          Visible = True
          ItemName = 'BtnFechar'
        end>
      OldName = 'Navega'#231#227'o'
      OneOnRow = True
      Row = 0
      ShowMark = False
      SizeGrip = False
      UseOwnFont = False
      UseRestSpace = True
      Visible = True
      WholeRow = False
    end
    object btnIncluir: TdxBarButton
      Action = ActIncluir
      Category = 0
      Glyph.Data = {
        
        888888800000000888888888888888888888}
    end
    object btnExcluir: TdxBarButton
      Action = ActExcluir
      Category = 0
      Glyph.Data = {
        
        0000000000000000008080008080008080008080008080000000}
    end
    object btnLocalizar: TdxBarButton
      Action = ActLocalizar
      Category = 0
      Glyph.Data = {
        
        D7D7D7D7AC56ACAC0000ACAC565656565656565656ACACAC0000}
    end
    object BtnSalvar: TdxBarButton
      Action = ActPost
      Category = 0
      Enabled = False
      Glyph.Data = {
        
        BFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBFBF}
    end
    object BtnCancelar: TdxBarButton
      Action = ActCancel
      Category = 0
      Enabled = False
      Glyph.Data = {
        
        8000808000808000808000808000808000808000808000808000}
    end
    object BtnFechar: TdxBarButton
      Action = ActFechar
      Align = iaRight
      Category = 0
      Glyph.Data = {
        
        0000000000000000000000000000000000008080008080000000}
      PaintStyle = psCaptionGlyph
    end
    object FecharAlteracao: TdxBarButton
      Action = ActFecharAlteracao
      Caption = 'Fechar ( F9 )     -   Localizar  -  F3'
      Category = 0
    end
    object dxBarBDBNavPrev: TdxBarDBNavButton
      Caption = 'ActPrior'
      Category = 1
      Enabled = False
      Hint = 'Ir ao registro anterior'
      Visible = ivAlways
      Glyph.Data = {
        
        7777777777777777777777777777777777777777777777777777}
      NavButton = dxbnPrior
    end
    object dxBarBDBNavNext: TdxBarDBNavButton
      Caption = 'ActNext'
      Category = 1
      Enabled = False
      Hint = 'Ir ao pr'#243'ximo registro'
      Visible = ivAlways
      Glyph.Data = {
        
        7777777777777777777777777777777777777777777777777777}
      NavButton = dxbnNext
    end
  end
  object Actions: TActionList
    Left = 281
    Top = 224
    object ActIncluir: TAction
      Tag = 1
      Caption = 'ActIncluir'
      Hint = 'Incluir um novo registro [Ins]'
      ShortCut = 45
      OnExecute = ActIncluirExecute
    end
    object ActAlterar: TAction
      Tag = 1
      Caption = 'ActAlterar'
    end
    object ActExcluir: TAction
      Tag = 1
      Caption = 'ActExcluir'
      Hint = 'Excluir o registro selecionado [Shift+Del]'
      ShortCut = 8238
      OnExecute = ActExcluirExecute
    end
    object ActLocalizar: TAction
      Caption = 'ActLocalizar'
      Hint = 'Localizar registros [F3]'
      ShortCut = 114
      OnExecute = ActLocalizarExecute
    end
    object ActPost: TAction
      Caption = 'ActPost'
      Hint = 'Salvar informa'#231#245'es'
      ShortCut = 121
      OnExecute = ActPostExecute
    end
    object ActCancel: TAction
      Caption = 'ActCancel'
      Hint = 'Cancelar informa'#231#245'es'
      ShortCut = 27
      OnExecute = ActCancelExecute
    end
    object ActFechar: TAction
      Caption = '&Fechar  '
      OnExecute = ActFecharExecute
    end
    object ActFecharAlteracao: TAction
      Caption = 'ActFecharAlteracao'
      ShortCut = 120
    end
  end
  object DataSource: TDataSource
    DataSet = DmEstoque.Orcamento_Mascara
    OnStateChange = DataSourceStateChange
    OnUpdateData = DataSourceUpdateData
    Left = 117
    Top = 224
  end
  object DsGrupo: TDataSource
    DataSet = DmEstoque.SelGrupo
    Left = 53
    Top = 280
  end
  object DsSubgrupo: TDataSource
    DataSet = DmEstoque.SelSubGrupo
    Left = 141
    Top = 280
  end
  object DsAgrupador: TDataSource
    DataSet = DmEstoque.Orcamento_Mascara_Nome
    OnStateChange = DataSourceStateChange
    Left = 229
    Top = 280
  end
end
