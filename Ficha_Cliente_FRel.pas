...
type
  TFRelFicha_Financeira = class(TForm)
    pnlClient: TPanel;
    Panel2: TPanel;
    RzBitBtn1: TcxButton;
    RzBitBtn2: TcxButton;
...
var
  FRelFicha_Financeira: TFRelFicha_Financeira;
...
procedure TFRelFicha_Financeira.IncLin;
begin
     Linha := Linha + 1;
...
procedure TFRelFicha_Financeira.FormCreate(Sender: TObject);
begin
...
procedure TFRelFicha_Financeira.ActFecharExecute(Sender: TObject);
begin
  Close;
end;

procedure TFRelFicha_Financeira.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  //
  dsClientes.DataSet.Close;

  Action      := caFree;
  FRelFicha_Financeira := Nil;
end;

procedure TFRelFicha_Financeira.cmbPessoaKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TFRelFicha_Financeira.PrintBeforeNewPage(Sender: TObject; Pagina: Integer);
begin
  // Rodapé...
...
procedure TFRelFicha_Financeira.PrintNewPage(Sender: TObject; Pagina: Integer);
begin
     // Cabeçalho...
...
procedure TFRelFicha_Financeira.FormShow(Sender: TObject);
begin
  //
...
procedure TFRelFicha_Financeira.ActPreviewExecute(Sender: TObject);
begin
  ...
  begin
       ...
       THEN BEGIN
            VER_FICHA_CLIENTE.Params.CreateParam ( FtInteger, 'PES', PtInputOutPut );

            VER_FICHA_CLIENTE.SQL.Add (' And PESSOA_FJ = :PES ');

            VER_FICHA_CLIENTE.ParamByName ('PES').asInteger := cmbCliente.LookupKeyValue ;
       END;
...
procedure TFRelFicha_Financeira.IMPRIMEFICHA ;
VAR
   IMP, STR: STRING;
   TOTAL : REAL ;
begin
...
procedure TFRelFicha_Financeira.edDtInicialEnter(Sender: TObject);
begin
     CorFundo ( Sender );
end;

procedure TFRelFicha_Financeira.cmbClienteExit(Sender: TObject);
begin
     TIRACorFundo ( Sender );
end;
