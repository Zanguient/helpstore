...
begin
  ...
  //
  If FrmGrades.FormStyle = fsMDIChild Then
...
procedure TFrmGrades.ActIncluirExecute(Sender: TObject);
begin
...
procedure TFrmGrades.ActExcluirExecute(Sender: TObject);
begin
...
procedure TFrmGrades.ActPostExecute(Sender: TObject);
begin
  Try
...
procedure TFrmGrades.ActCancelExecute(Sender: TObject);
begin
...
procedure TFrmGrades.FormCreate(Sender: TObject);
begin
  //
...
procedure TFrmGrades.ActLocalizarExecute(Sender: TObject);
begin
  DMApp.Localizar(Datasource);
end;

procedure TFrmGrades.dxDBEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmGrades.pnlClientResize(Sender: TObject);
begin
  b2.Width := pnlClient.Width - 17;
  LblRegistros.Left := pnlClient.Width - 11 - LblRegistros.Width;
end;

procedure TFrmGrades.DataSourceStateChange(Sender: TObject);
begin
  If DataSource.State in dsEditModes Then
     Begin
...
procedure TFrmGrades.ActFecharExecute(Sender: TObject);
begin
     Close;
end;

procedure TFrmGrades.DataSourceDataChange(Sender: TObject; Field: TField);
begin
  LblRegistros.Caption := IntToStr(Datasource.DataSet.Recno) + '/' + IntToStr(Datasource.Dataset.RecordCount); 
end;

procedure TFrmGrades.EdInicialEnter(Sender: TObject);
begin
     CORFUNDO( SENDER );
end;

procedure TFrmGrades.EdInicialExit(Sender: TObject);
begin
     //TROCA A COR DE FUNDO
     TIRACORFUNDO( SENDER );
end;

procedure TFrmGrades.BtnMaterialClick(Sender: TObject);
begin
  //
...
procedure TFrmGrades.BtnCorClick(Sender: TObject);
begin
  //
  If ActAlterar.Tag = 0 Then
     Exit;
...
procedure TFrmGrades.ActLookUpExecute(Sender: TObject);
begin
...
procedure TFrmGrades.BtnPerfilClick(Sender: TObject);
begin
  //
  If ActAlterar.Tag = 0 Then
     Exit;
...
procedure TFrmGrades.ActNumeroExecute(Sender: TObject);
begin
  ...
  begin
    if (length(trim(EdNumero.text)) > 5) then
    begin
      messagedlg('Campo número não pode exceder 5 posições', MtError, [mbok],0);
      EdNumero.SetFocus ;
      exit;
    end;

    DmApp.inserir_Num_Grade(DMCadastros.GradesProduto.Value,
                            DMCadastros.GradesGrade.value,
                           (trim(EdNumero.text)));
    DsNumeros.DataSet.Close;
    DsNumeros.DataSet.Open;
    EdNumero.Clear;
...
procedure TFrmGrades.ActAlterarExecute(Sender: TObject);
begin
  If DataSource.State in dsEditModes Then
     Exit;
  Try
