...
type
  TFRelListaPrecosIndexada = class(TForm)
    pnlClient: TPanel;
    Panel2: TPanel;
    RzBitBtn1: TcxButton;
    RzBitBtn2: TcxButton;
    ...
    Label7: TcxLabel;
    EdTipo: TdxPickEdit;
    stgFRelListaPrecosIndexada: TcxPropertiesStore;
...
var
  FRelListaPrecosIndexada: TFRelListaPrecosIndexada;
...
procedure TFRelListaPrecosIndexada.FormCreate(Sender: TObject);
begin
...
procedure TFRelListaPrecosIndexada.ActFecharExecute(Sender: TObject);
begin
  Close;
end;

procedure TFRelListaPrecosIndexada.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  //
  dsGrupos.DataSet.Close;
  //
  Action := caFree;
  FRelListaPrecosIndexada := Nil;
end;

procedure TFRelListaPrecosIndexada.cmbPessoaKeyDown(Sender: TObject;
...
procedure TFRelListaPrecosIndexada.PrintBeforeNewPage(Sender: TObject; Pagina: Integer);
begin
  // Rodapé...
...
procedure TFRelListaPrecosIndexada.PrintNewPage(Sender: TObject; Pagina: Integer);
begin
  // Cabeçalho...
  ...
  Print.ImpF(08, 01, '                                                                                                                Prc. Unit.    Prc. Unit.', [Comp17]);
  Print.ImpF(09, 01, '         Codigo Nome                                               Unid.  Qtdade      Custo U$      Custo R$       a Prazo       a Vista', [Comp17]);
  Print.ImpF(10, 01, '--------------- -------------------------------------------------- ----- ------- ------------- ------------- ------------- -------------', [Comp17]);
  linha  := 11;
end;

procedure TFRelListaPrecosIndexada.FormShow(Sender: TObject);
begin
  //
...
procedure TFRelListaPrecosIndexada.ActPreviewExecute(Sender: TObject);
Var
   VENDA_VISTA, VENDA_PRAZO, CUSTO_REAL, COTACAO: REAL;
begin
  ...
  SubGrupo := 0;
  //
  Print.TitulodoRelatorio := 'Lista de Precos Indexada - Valida de ' + edDtInicial.Text + ' a ' + edDtFinal.Text;
  //
  ...
  COTACAO := DMAPP.COTACAO_DOLAR ( DMAPP.Data_Servidor );
  ...
  While Not DMRelatorios.ListaUnid.Eof Do
        Begin
          ...
          If SubGrupo <> DMRelatorios.ListaUnidSUBGRUPO.Value then
             ...
             CUSTO_REAL  := DMRelatorios.ListaUnidCUSTO_INDEXADO.VALUE * COTACAO ;

             VENDA_VISTA := CUSTO_REAL + (( CUSTO_REAL * DMRelatorios.ListaUnidMARGEM_BRUTA.VALUE ) / 100 );

             VENDA_PRAZO := VENDA_VISTA - (( VENDA_VISTA * edDesconto.VALUE ) / 100 );
             ...
             Print.ImpF(Linha, 01, Format('%15s %-50s %-5s %7s %13s %13s %13s %13s',
                                       [
                                        ...
                                        DMRelatorios.ListaUnidCUSTO_INDEXADO.Text,
                                        FORMATFLOAT('###,###,##0.00', CUSTO_REAL ),
                                        FORMATFLOAT('###,###,##0.00', VENDA_VISTA ),
                                        FORMATFLOAT('###,###,##0.00', VENDA_PRAZO )
                                       ]), [Comp17]);
             //
...
procedure TFRelListaPrecosIndexada.EdTipoEnter(Sender: TObject);
begin
     CORFUNDO ( SENDER );
end;

procedure TFRelListaPrecosIndexada.EdTipoExit(Sender: TObject);
begin
     TIRACORFUNDO ( SENDER );
end;
