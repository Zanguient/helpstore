...
uses
  ...
  dxSkinValentine, dxSkinXmas2008Blue, cxButtons, cxLabel, IBCustomDataSet,
  IBQuery;
...
type
  TFrm_Localizar_Cliente = class(TForm)
    ...
    stgFrm_Localizar_Cliente3: TcxPropertiesStore;
    Panel1: TPanel;
    LblTitulo: TcxLabel;
    Bevel2: TBevel;
    Label6: TcxLabel;
    ...
    SelPessoasFJNOME_TECNICO: TIBStringField;
    GRIDREGIAO: TdxDBGridMaskColumn;
    GRIDNOME_REGIAO: TdxDBGridMaskColumn;
...
begin
  If (SelPessoasFJPESSOA_FJ.asString <> '') then
   CampTrecho :=  GetTrecho;

  CampNome :=  SelPessoasFJNOME_RAZAO.value;
  ModalResult:=mrOk;
end;
...
begin
   ...
   If SelPessoasFJCODIGO.AsString = '' Then
     Application.Messagebox('Cliente não encontrado','Aviso', mb_iconquestion + mb_ok);
...
begin
  sqlOriginal := SelPessoasFJ.sql.text;
  if not (EdCodigo.Text = '') then
          Frm_Localizar_Cliente.EdCodigoExit(Sender);
end;

procedure TFrm_Localizar_Cliente.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  SelPessoasFJ.sql.text := sqlOriginal;
end;
