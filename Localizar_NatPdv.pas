...
type
  TFrmLocNatPdv = class(TForm)
    ...
    stgFrmLocNatPdv: TcxPropertiesStore;
...
function TFrmLocNatPdv.GetTrecho: String ;
begin
     Result := DSource.DataSet.FieldByName ('CODIGO').AsString ;
end;

procedure TFrmLocNatPdv.FormShow(Sender: TObject);
begin
     ...
     THEN BEGIN
          ...
          Label6.Font.Color      := STRTOINT(TRIM(DMAPP.PDV_COR_FONTE));
          Label1.Font.Color      := STRTOINT(TRIM(DMAPP.PDV_COR_FONTE));
...
procedure TFrmLocNatPdv.GridKeyPress(Sender: TObject; var Key: Char);
begin
     If Key = #13
     then begin
          ModalResult:=mrOk;
...
procedure TFrmLocNatPdv.GridDblClick(Sender: TObject);
begin
     ModalResult:=mrOk;
end;

procedure TFrmLocNatPdv.GRIDKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
    If Key = 13
     then begin
          ModalResult:=mrOk;
...
procedure TFrmLocNatPdv.ActOkExecute(Sender: TObject);
begin
     ModalResult:=mrOk;
end;

procedure TFrmLocNatPdv.ActCancelarExecute(Sender: TObject);
begin
     ModalResult:=mrCancel;
end;

procedure TFrmLocNatPdv.Ordenar;
begin
     If RdNome.Checked
     then begin
...
procedure TFrmLocNatPdv.EdCodigoKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TFrmLocNatPdv.EdCodigoExit(Sender: TObject);
Var
   Aux : String;
begin
     TiraCorFundo ( Sender );
...
procedure TFrmLocNatPdv.EdNomeExit(Sender: TObject);
begin
     TiraCorFundo ( Sender );
...
procedure TFrmLocNatPdv.GRIDEnter(Sender: TObject);
begin
     ActOk.ShortCut       := 13 ;
     ActCancelar.ShortCut := VK_F12 ;
end;

procedure TFrmLocNatPdv.GRIDExit(Sender: TObject);
begin
     ActOk.ShortCut       := VK_F12 ;
     ActCancelar.ShortCut := 27 ;
end;

procedure TFrmLocNatPdv.FormCreate(Sender: TObject);
begin
     If UPPERCASE(DmApp.ORDENA_VENDA) <> 'NOME'
     then begin
          DSource.DataSet := DMLocalizarProd.Produtos ;
...
procedure TFrmLocNatPdv.EdCodigoEnter(Sender: TObject);
begin
     CorFundo( Sender );
...
procedure TFrmLocNatPdv.EdNomeEnter(Sender: TObject);
begin
     CorFundo( Sender );
