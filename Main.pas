...
type
  TFrmMain = class(TForm)
    ...
    btnGeraPreventiva: TdxBarButton;
    ...
    procedure btnGeraPreventivaClick(Sender: TObject);
...
procedure TFrmMain.TrimAppMemorySize;
var
  MainHandle : THandle;
begin
  try
...
begin
    if Not(DMApp.Verificar_Login(FileName(Application.ExeName), 'frmCadClientes', True)) Then
     Exit;
...
procedure TFrmMain.btnGeraPreventivaClick(Sender: TObject);
begin
  If FrmMain.MDIChildCount > 1 Then
     Exit;

  if not(DMApp.Verificar_Login(FileName(Application.ExeName), 'frmGeraVisitaPreventiva', True)) then
    Exit;

  if DMApp.SelecionarEmpresa = 'N' then
    Exit;

  if frmGeraVisitaPreventiva = nil  then
  begin
    Application.ProcessMessages;
    frmGeraVisitaPreventiva := TfrmGeraVisitaPreventiva.Create(Self);
    frmGeraVisitaPreventiva.Showmodal;
    frmGeraVisitaPreventiva.Free;
    frmGeraVisitaPreventiva := Nil;
  end;
...
begin
  ...
  if not(DMApp.Verificar_Login(FileName(Application.ExeName), 'frmCadHelp', True)) then
    Exit;

  if DMApp.SelecionarEmpresa = 'N' then
    Exit;

  if frmCadHelp = nil  then
  begin
    Application.ProcessMessages;
    frmCadHelp := TfrmCadHelp.Create(Self);
    frmCadHelp.Showmodal;
    frmCadHelp.Free;
    frmCadHelp := Nil;
  end;
end;

procedure TFrmMain.btnControleTonnerClick(Sender: TObject);
begin
   If FrmMain.MDIChildCount > 1 Then
     Exit;

  if not(DMApp.Verificar_Login(FileName(Application.ExeName), 'frmCadControleTonner', True)) then
    Exit;

  if DMApp.SelecionarEmpresa = 'N' then
    Exit;

  if frmCadControleTonner = nil  then
  begin
    Application.ProcessMessages;
    frmCadControleTonner := TfrmCadControleTonner.Create(Self);
    frmCadControleTonner.Showmodal;
    frmCadControleTonner.Free;
    frmCadControleTonner := Nil;
  end;
end;

procedure TFrmMain.btnListaContTonnerClick(Sender: TObject);
begin
  If FrmMain.MDIChildCount > 1 Then
     Exit;

  if not(DMApp.Verificar_Login(FileName(Application.ExeName), 'frmListaControleTonner', True)) then
    Exit;

  if DMApp.SelecionarEmpresa = 'N' then
    Exit;

  if frmListaControleTonner = nil  then
  begin
    Application.ProcessMessages;
    frmListaControleTonner := TfrmListaControleTonner.Create(Self);
    frmListaControleTonner.Showmodal;
    frmListaControleTonner.Free;
    frmListaControleTonner := Nil;
  end;
end;
