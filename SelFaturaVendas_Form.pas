...
uses Vendas_DM,
  ...
  Cadastros_Dm2, dateutils, UntExportaXml, UntCartaCorrecao;
...
procedure TFrmSelFaturaVendas.cxButton1Click(Sender: TObject);
begin
...
procedure TFrmSelFaturaVendas.ActCartaCorrecaoExecute(Sender: TObject);
begin
  if (DataSource.DataSet.FieldByName('CODIGO').Value = null) or (DataSource.DataSet.FieldByName('NFE_XML').Value = null) then
  begin
    Application.MessageBox('Selecione uma  venda com NFe para emitir a Carta de Correção Eletrônica.','Aviso',MB_YESNO + MB_ICONEXCLAMATION);
    Exit;
  end;
  FrmCartaCorrecao  := TFrmCartaCorrecao.Create(Self);
  FrmCartaCorrecao.Venda := DataSource.DataSet.FieldByName('CODIGO').Value;
