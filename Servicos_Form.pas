...
begin
  ...
  If FrmServicos.FormStyle = fsMDIChild Then
...
begin
  If MessageBox(Handle,'Tem certeza que deseja Excluir este Serviço ?',
                       'Atenção !!!',MB_YESNO+MB_ICONQUESTION+MB_DEFBUTTON2)=IDYES Then
     DataSource.Dataset.Delete;
end;

procedure TFrmServicos.ActPostExecute(Sender: TObject);
begin
  Try
...
procedure TFrmServicos.ActCancelExecute(Sender: TObject);
begin
...
procedure TFrmServicos.ActListagemExecute(Sender: TObject);
begin
  //
  DMListagens := TDMListagens.Create(Self);
  DMListagens.lsServicos(DataSource.DataSet);
  DMListagens.Free;
  DMListagens := Nil;
end;

procedure TFrmServicos.ActLocalizarExecute(Sender: TObject);
begin
  try
        ...
        Frm_Localizar_Generico.LblTitulo.Caption := 'Localizar Serviços' ;
...
procedure TFrmServicos.dxDBEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmServicos.pnlClientResize(Sender: TObject);
begin
  b2.Width := pnlClient.Width - 17;
  LblRegistros.Left := pnlClient.Width - 11 - LblRegistros.Width;
end;

procedure TFrmServicos.DataSourceStateChange(Sender: TObject);
begin
  If DataSource.State in dsEditModes Then
     Begin
...
procedure TFrmServicos.ActFecharExecute(Sender: TObject);
begin
  If FrmServicos.FormStyle = fsMDIChild Then
...
procedure TFrmServicos.DataSourceDataChange(Sender: TObject; Field: TField);
begin
  LblRegistros.Caption := IntToStr(Datasource.DataSet.Recno) + '/' + IntToStr(Datasource.Dataset.RecordCount);
end;

procedure TFrmServicos.ActAlterarExecute(Sender: TObject);
begin
  If DataSource.State in dsEditModes Then
     Exit;
  Try
...
procedure TFrmServicos.EdInicialEnter(Sender: TObject);
begin
     CORFUNDO ( SENDER );
end;

procedure TFrmServicos.EdInicialExit(Sender: TObject);
begin
     TIRACORFUNDO ( SENDER );
end;
