...
procedure TFrmTransportadoras.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  //
...
{  If DMCadastros.Tag = 0 Then
     ...
     End;}
  //
  If FrmTransportadoras.FormStyle = fsMDIChild Then
...
procedure TFrmTransportadoras.ActPostExecute(Sender: TObject);
begin
  Try
    if trim(edtCPFCNPJ.text) = '' then
    begin
      Application.MessageBox('Defina um CPF ou CNPJ','Aviso',mb_iconerror + mb_ok);
      edtCPFCNPJ.SetFocus;
      exit;
    end;
...
procedure TFrmTransportadoras.ActCancelExecute(Sender: TObject);
begin
...
procedure TFrmTransportadoras.FormShow(Sender: TObject);
begin
...
procedure TFrmTransportadoras.ActListagemExecute(Sender: TObject);
begin
  //
  DMListagens := TDMListagens.Create(Self);
  DMListagens.lsTransportadoras(DataSource.DataSet);
  DMListagens.Free;
  DMListagens := Nil;
end;
...
begin
  try
        ...
        Frm_Localizar_Generico.LblTitulo.Caption := 'Localizar Transportadoras' ;
...
procedure TFrmTransportadoras.dxDBEdit1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TFrmTransportadoras.pnlClientResize(Sender: TObject);
begin
  b2.Width := pnlClient.Width - 17;
  LblRegistros.Left := pnlClient.Width - 11 - LblRegistros.Width;
end;

procedure TFrmTransportadoras.DataSourceStateChange(Sender: TObject);
begin
  If DataSource.State in dsEditModes Then
     Begin
...
procedure TFrmTransportadoras.ActFecharExecute(Sender: TObject);
begin
  If FrmTransportadoras.FormStyle = fsMDIChild Then
...
procedure TFrmTransportadoras.DataSourceDataChange(Sender: TObject; Field: TField);
begin
...
procedure TFrmTransportadoras.EdInicialEnter(Sender: TObject);
begin
     CORFUNDO ( SENDER );
end;

procedure TFrmTransportadoras.EdInicialExit(Sender: TObject);
begin
     TIRACORFUNDO ( SENDER );
end;
