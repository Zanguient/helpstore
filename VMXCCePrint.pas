unit VMXCCePrint;interface
uses Windows, RpDefine, RpBase, RpSystem, Graphics, SysUtils, Classes, RpRenderPDF, RpDevice, MaskUtils, Funcoes, Forms;
...
begin
  ...
  TitlePreview:='CARTA CORREÇÃO ELETRÔNICA'; // Coloca Título na Janela de visualização...
...
begin
   ...
   impBox(170,65,200,73,'VERSÃO DO EVENTO',LeitorCCe.valor('ProcEventoNFe,evento,infevento','verEvento'),al_center);  aExt := ExtractFileExt(FXMLCCe);
  aName := ExtractFileName(FXMLCCe);
  if aExt <> '' then
  begin
    aPos := Pos(aExt,aName);
    if aPos > 0 then
      Delete(aName,aPos,Length(aExt));
  end;

   impBox(10,73,200,81,'CARTA DE CORREÇÃO ELETRÔNICA',aName,al_left);
   ...
   impBoxMemo(10,169,200,227,'',LeitorCCe.valor('ProcEventoNFe,evento,Items[0],infevento,detEvento','xcorrecao'),13,' - *');
