...
type
  TFRelVendasCliente = class(TForm)
    pnlClient: TPanel;
    Panel2: TPanel;
    RzBitBtn1: TcxButton;
    RzBitBtn2: TcxButton;
...
var
  FRelVendasCliente: TFRelVendasCliente;
...
procedure TFRelVendasCliente.FormCreate(Sender: TObject);
begin
...
procedure TFRelVendasCliente.IncLin;
begin
     Linha := Linha + 1;
...
procedure TFRelVendasCliente.ActFecharExecute(Sender: TObject);
begin
  Close;
end;

procedure TFRelVendasCliente.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  //
  Action := caFree;
  FRelVendasCliente := Nil;
end;

procedure TFRelVendasCliente.cmbPessoaKeyDown(Sender: TObject;
...
procedure TFRelVendasCliente.PrintBeforeNewPage(Sender: TObject; Pagina: Integer);
begin
  // Rodapé...
...
procedure TFRelVendasCliente.PrintNewPage(Sender: TObject; Pagina: Integer);
begin
  // Cabeçalho...
  ...
  Else // Analitico
     Begin
       ...
       Print.ImpF(06,32,'Un', [Comp17]);
       Print.ImpF(06,34,'SubUnidade', [Comp17]);
...
procedure TFRelVendasCliente.FormShow(Sender: TObject);
begin
  //
...
procedure TFRelVendasCliente.ActPreviewExecute(Sender: TObject);
Var
    Cod_Cliente: Integer;
...
begin
  ...
  Else
     Begin
        DMRelatorios.Fat_VendasCli.ParamByName('DTINIC').asString:= Ini.Text;
        DMRelatorios.Fat_VendasCli.ParamByName('DTFINAL').asString:= Fim.Text;
     End;
  If CmbCliente.Text = '' Then
     DMRelatorios.Fat_VendasCli.ParamByName('CLIENTE').asString:= '0'
  Else
     DMRelatorios.Fat_VendasCli.ParamByName('CLIENTE').Value:= DsCliente.DataSet.fieldByName('PESSOA_FJ').Value;
  ...
  Else
     Begin
       lblStatus.Visible := False ;
       DMRelatorios.Fat_VendasCli.fetchall ;
       Anda.Max := DMRelatorios.Fat_VendasCli.RecordCount;
       Anda.Position := 0;
       Anda.Visible  := TRUE ;
     End;
  //
  print.TitulodoRelatorio:='Relatorio de Vendas por Cliente';
  print.Abrir;
  //
  Total_Cliente:= 0;
  Total_Geral  := 0;
  Cod_Cliente  := 0;
  //
  DMRelatorios.Fat_VendasCli.First;
  While Not DMRelatorios.Fat_VendasCli.Eof Do
  Begin
          //
          If Cod_Cliente <> DmRelatorios.Fat_VendascliPESSOA_FJ.Value Then
             Begin
               Print.ImpF(Linha,01,'Cliente : '+ DmRelatorios.Fat_VendasCliPESSOA_RAZAO.Text,[Comp17, Negrito]);
               Cod_Cliente := DmRelatorios.Fat_VendasCliPESSOA_FJ.Value;
               IncLin;
               IncLin;
               Total_Cliente:= 0;
             End;
          //
          Print.ImpF(Linha,  01, DMRelatorios.Fat_VendascliCODIGO.Text,[Comp17]);
          Print.ImpF(Linha,  07, DMRelatorios.Fat_VendascliDATA.Text,[Comp17]);
          Print.ImpF(Linha,  15, DMRelatorios.Fat_VendascliNOME_VENDEDOR.Text,[Comp17]);
          Print.ImpF(Linha,  30, DMRelatorios.Fat_VendascliNATUREZA.Text,[Comp17]);
          Print.ImpVAL(Linha,43, '###,##0.00',DMRelatorios.Fat_VendascliNUM_NF.Value,[Comp17]);
          Print.ImpVal(Linha,53, '###,##0.00',DMRelatorios.Fat_VendascliTOTAL.value,[Comp17]);
          Print.ImpVal(Linha,63, '###,##0.00',DMRelatorios.Fat_VendascliDESC_ACRES.value,[Comp17]);
          Print.ImpVal(Linha,73, '###,##0.00',DMRelatorios.Fat_VendascliVLRLIQUIDO.value,[Comp17]);
          ...
          Begin
               ...
               DMRelatorios.Fat_VendasItens.ParamByName('CODIGO').Value := DMRelatorios.Fat_VendascliCODIGO.Value;
               ...
               Begin
                       ...
                       Print.ImpF(Linha  ,32,DMRelatorios.Fat_VendasItensUNIDADE.Text, [Comp17]);
                       Print.ImpF(Linha  ,34,DMRelatorios.Fat_VendasItensSUBUNIDADE.Text, [Comp17]);
                     ...
                     End;
          End;
          Total_Geral:= Total_Geral + DMRelatorios.Fat_VendascliVLRLIQUIDO.Value;
          Total_Cliente:= Total_Cliente + DMRelatorios.Fat_VendascliVLRLIQUIDO.Value;
          //
          Anda.Position := Anda.Position + 1;
          DMRelatorios.Fat_Vendascli.Next;
          //
          IncLin;
          //
          If (Cod_Cliente <> DmRelatorios.Fat_VendascliPESSOA_FJ.Value) Or
             (DMRelatorios.Fat_Vendascli.Eof) Then
             Begin
        ...
        End;
  //
  Print.ImpF(Linha , 55, 'Total Geral', [Comp17, Negrito]);
...
procedure TFRelVendasCliente.IniEnter(Sender: TObject);
begin
     CORFUNDO(SENDER);
end;

procedure TFRelVendasCliente.IniExit(Sender: TObject);
Var
   Data: TDateTime ;
begin
...
procedure TFRelVendasCliente.FimExit(Sender: TObject);
VAR
   DATA: TDATETIME;
begin
     TIRACORFUNDO( SENDER );
