...
type
  TfrmCadContratoAtendimento = class(TfrmCadPadraoMaster)
    ...
    cxLabel28: TcxLabel;
    dtEditDet1TEMPO_RESPOSTA_PREV: TIntegerField;
    dtListDet1TEMPO_RESPOSTA_PREV: TIntegerField;
    TVDet1TEMPO_RESPOSTA_PREV: TcxGridDBBandedColumn;
    ...
    procedure dtEditDet1ATIVOChange(Sender: TField);
  private
...
uses UntCadTipoContrato, Application_DM, untCadClientes;
...
begin
  ...
  //até a definição do cliente, conenciona-se o tempo resposta de prev. padrão o de 15 dias
  dtEditDet1TEMPO_RESPOSTA_PREV.value := 15;

  // Seleciona apenas equipamentos disponíveis
  QryProduto.Close;
  QryProduto.SQL.Clear;
  QryProduto.SQL.Text := 'select p.codigo, p.serie, (coalesce(p.serie,'''')||'' - ''||p.nome) nome from est_produtos p ' +
    ' where p.cnpj = :cnpj and coalesce(p.ativo,''N'') = ''S'' and (( ' +
    ' select count(*) from ofc_contratos_itens osi ' +
    ' where osi.cnpj = p.cnpj and coalesce(osi.ativo, ''N'') = ''S'' and osi.cod_produto = p.codigo) = 0) and coalesce(p.servico,''N'') = ''N''';
  QryProduto.Open;
end;
...
procedure TfrmCadContratoAtendimento.cxButton1Click(Sender: TObject);
begin
  inherited;
  CadastroLookup(TfrmCadClientes,dtEdit,'COD_CLIENTE',QryCliente);
end;

procedure TfrmCadContratoAtendimento.dtEditDet1ATIVOChange(Sender: TField);
begin
  inherited;
  // Seleciona apenas equipamentos disponíveis
  QryProduto.Close;
  ...
  begin
    QryProduto.SQL.Clear;
    QryProduto.SQL.Text := 'select p.codigo, p.serie, (coalesce(p.serie,'''')||'' - ''||p.nome) nome from est_produtos p' +
      ' where p.cnpj = :cnpj and coalesce(p.ativo,''N'') = ''S''';
  end;
  QryProduto.Open;
end;
