...
type
  TfrmCadLeituras = class(TfrmCadPadraoMaster)
    ...
    cxLabel14: TcxLabel;
    dtListDESC_ACRESC_TOTAL: TFloatField;
    TVRegistroDESC_ACRESC_TOTAL: TcxGridDBBandedColumn;
    dtEditDESC_ACRESC_TOTAL: TFloatField;
    ...
    procedure dtEditDESC_ACRESC_TOTALValidate(Sender: TField);
    procedure dtEditCONTRATOValidate(Sender: TField);
...
procedure TfrmCadLeituras.dtEditDESC_ACRESC_TOTALValidate(Sender: TField);
var
  Posicao: Integer;
  MsgAcres: String;
begin
  ...
  Posicao := Pos(MsgAcres, dtEditOBSERVACAO.AsString);

  if Posicao > 0 then
    dtEditOBSERVACAO.AsString := Copy(dtEditOBSERVACAO.AsString, 1, Posicao -1);
    
  if dtEditDESC_ACRESC_TOTAL.Value < 0 then
  begin
    Application.MessageBox('Não é permitido desconto!','Erro', mb_ok + mb_iconerror);
    dtEditDESC_ACRESC_TOTAL.Clear;
    Exit;
  end;
  ...
  if Pos(#013, Copy(dtEditOBSERVACAO.AsString, Length(dtEditOBSERVACAO.AsString), Length(dtEditOBSERVACAO.AsString) -1)) > 0 then
    dtEditOBSERVACAO.AsString := dtEditOBSERVACAO.AsString + MsgAcres + dtEditDESC_ACRESC_TOTAL.AsString + ' REAIS'
  else
    dtEditOBSERVACAO.AsString := dtEditOBSERVACAO.AsString + #013 + MsgAcres + dtEditDESC_ACRESC_TOTAL.AsString + ' REAIS';

  dtEditVALOR_LEITURA.Value := dtEditCTR_VALOR.AsFloat + dtEditVALOR_TOTAL_COPIAS_EXCENTE.AsFloat + dtEditDESC_ACRESC_TOTAL.Value;
end;

procedure TfrmCadLeituras.dtEditCONTRATOValidate(Sender: TField);
var
  AuxSql: String;
begin
  inherited;
  ...
  AuxSql := 'select grp.codigo from ofc_grp_contratos grp where grp.dt_fim < (select data from data_servidor) and grp.codigo = ' + dtEditCONTRATO.AsString;
  if RetornaValor(AuxSql) <> null then
  begin
    Application.MessageBox('Vigência do contrato de cópias expirada.','Erro', mb_ok + mb_iconerror);
    aTfrmCadContratoCopias.SetFocus; 
    Sender.Clear;
  end;
  //
end;

procedure TfrmCadLeituras.ActCadLookupExecute(Sender: TObject);
begin
  inherited;
  if aTfrmCadContratoCopias.Focused then
    BtnContratoCopias.Click;
end;
