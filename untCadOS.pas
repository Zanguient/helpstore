...
type
  TfrmCadOS = class(TfrmCadPadraoMaster)
    ...
    dtListMTC_PADRAO_PREVENTIVA: TIBStringField;
    TVRegistroMTC_PADRAO_PREVENTIVA: TcxGridDBBandedColumn;
    dtListCP_TEMPO_RESPOSTA_PREV: TIntegerField;
    TVRegistroCP_TEMPO_RESPOSTA_PREV: TcxGridDBBandedColumn;
...
begin
  ...
  begin
    ...
    SelSeries.Open;
        ...
        //Sanniel- Verifica se o contrato é vigente

    AuxSql := 'select cti.codigo from ofc_contratos_itens cti ' +
              'left join est_produtos prd on (prd.codigo = cti.cod_produto and cti.cnpj = prd.cnpj) ' +
              'where prd.serie = ''' + SelSeriesCOD_EQUIPAMENTO.AsString +''' and cti.ativo = ''S'' and ((cast(cti.dt_vigencia_final as date)) < (select cast(data as date) from data_servidor))';
...
procedure TfrmCadOS.TVRegistroOFC_CODIGOCustomDrawCell(
  ...
  AViewInfo: TcxGridTableDataCellViewInfo; var ADone: Boolean);
var
  ValueGasto, ValueResposta,PadraoVisitaPreventiva : Variant;
begin
  //Procedimento irá pintar de vermelho a linha em que o atendimento tenha 'estourado' o tempo resposta delimitado
  //no contrato de atendimento, lembrando que se o campo Motivo do Chamado for do tipo 'Padrão' visita preventiva
  //o tempo resposta a ser considerado será o do campo especifico para este tipo de OS, também delimitado no item do contrato de atendimento
  ...
  begin
     ...
     //verificando se o motivo do chamado é 'Padrão' de visita preventiva
     PadraoVisitaPreventiva := TVRegistro.ViewData.Records[AViewInfo.GridRecord.Index].Values[TVRegistroMTC_PADRAO_PREVENTIVA.Index];

     if PadraoVisitaPreventiva = 'S' then
        ValueResposta := TVRegistro.ViewData.Records[AViewInfo.GridRecord.Index].Values[TVRegistroCP_TEMPO_RESPOSTA.Index]
     else
        ValueResposta := TVRegistro.ViewData.Records[AViewInfo.GridRecord.Index].Values[TVRegistroCP_TEMPO_RESPOSTA_PREV.Index];
...
begin
  ...
  if (dtEditDet2.State in [dsedit, dsinsert]) and ((dtEditDet2DT_INICIO.IsNull)  OR (dtEditDet2HR_INICIO.IsNull) OR (dtEditDet2DT_FIM.IsNull) OR (dtEditDet2HR_FIM.IsNull))   then
  begin
